...
ダーツプレイヤーにとって、自身のセッティング（バレル・シャフト・フライト・チップの組み合わせ）の管理や、練習スタッツの記録・可視化は日常的なニーズである。しかし、既存サービスでは以下の課題が解決されていない。

- **セッティング管理が分散している**: 写真やメモで断片的に記録しており、過去のセッティングを振り返りにくい。パーツごとの詳細スペック（0.1mm / 0.1g 単位の長さ・重量）を記録・比較できるサービスは存在しない
- **スタッツの記録が手間**: DARTSLIVE 公式アプリでは過去データの推移比較が限定的で、自分の成長を俯瞰しにくい
- **マシンごとにデータが分断**: DARTSLIVE と PHOENIX は別会社・別アプリであり、両方プレイするユーザーは統合的にデータを確認する手段がない
- **練習の成長を客観的に分析する手段がない**: レーティング推移だけでは把握できないスキルの偏りや改善ポイントを可視化する仕組みが求められている
- **バレル選びの情報が分散**: スペック比較をするために複数の EC サイトを横断する必要がある。既存のバレル検索サイトはフィルターの粒度が粗く、重量・最大径・全長などを細かい範囲で絞り込めない
- **ショップ管理が断片的**: ダーツショップの情報が散在しており、訪問済み / 未訪問の管理やお気に入り店舗の整理が難しい。禁煙 / 分煙での絞り込みや駅の路線からの検索は既存サービスにない
...
本プロジェクトは以下の 2 つの目的を持つ。

1. **実用ツール**: 自身を含むダーツプレイヤーが日常的に使えるアプリケーション
2. **技術ポートフォリオ**: Web エンジニアとしての設計力・実装力を示す成果物

### 1.3 競合分析・差別化

既存サービスとの比較と Darts Lab の差別化ポイントを以下に整理する。

#### バレル検索

| 観点           | 既存サービス（EC サイト・ポータル）   | Darts Lab                                          |
| -------------- | ------------------------------------- | -------------------------------------------------- |
| フィルター粒度 | 重量は「16〜18g」のような大まかな範囲 | 0.1g / 0.1mm 単位のスライダー指定                  |
| スペック比較   | 基本的に単品ページのみ                | フィルター結果を一覧でスペック比較可能             |
| レコメンド     | なし                                  | 独自レコメンドエンジンによる「あなたへのおすすめ」 |
| 診断           | なし                                  | バレル診断クイズ（好みの投げ方・グリップから提案） |

#### マイショップ

| 観点                  | 既存サービス                | Darts Lab                                          |
| --------------------- | --------------------------- | -------------------------------------------------- |
| 禁煙 / 分煙フィルター | なし                        | 禁煙・分煙・喫煙可で絞り込み可能                   |
| 路線検索              | なし                        | 駅名・路線名から沿線のショップを検索可能           |
| お気に入り登録        | ホームショップを 1 店舗のみ | 複数ブックマーク + リスト分類 + タグ + メモ + 評価 |
| 訪問管理              | なし                        | 訪問済み / 未訪問フィルター + カード上のバッジ表示 |

#### スタッツ・分析

| 観点             | 既存サービス（DARTSLIVE / PHOENIX 公式アプリ） | Darts Lab                                                                         |
| ---------------- | ---------------------------------------------- | --------------------------------------------------------------------------------- |
| データ統合       | DARTSLIVE と PHOENIX は別会社・別アプリ        | 両方のデータを 1 つの画面で統合閲覧                                               |
| 分析の深さ       | レーティング推移 + 直近成績の確認程度          | レーダーチャート・トレンド分析・セッション疲労分析・AI レコメンドなど多角的に強化 |
| セッティング比較 | なし                                           | 0.1mm / 0.1g 単位での定量比較（従来は感覚だより）                                 |

### 1.4 完全オリジナル機能

既存のダーツ関連サービスにはない、Darts Lab 独自の機能を以下に整理する。

| 機能                     | 概要                                                                               | 実装の狙い                                             |
| ------------------------ | ---------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **LINE レポート配信**    | LINE Messaging API 経由で日次スタッツ・コンディション記録を配信                    | プッシュ型の情報提供で日常的な利用を促進               |
| **目標トラッキング**     | レーティング目標・アワード目標を設定し、進捗をダッシュボードで可視化               | 成長のモチベーション維持                               |
| **ヘルスケア連携**       | Apple HealthKit から歩数・睡眠データを取得し、コンディションとスタッツの相関を分析 | ダーツ以外の身体データとパフォーマンスの関連を発見     |
| **セッティング履歴**     | 使用したセッティングの開始・終了日を記録し、時系列で管理                           | 「あの時期に使っていたセッティング」を正確に振り返れる |
| **セッティング定量比較** | 2 つのセッティングをパーツ単位で 0.1mm / 0.1g 単位で並列比較                       | 従来の「感覚」による比較を定量データで裏付け           |
| **高度な分析機能**       | スキルレーダー・プレイヤー DNA・COUNT-UP 深掘り分析・セッション疲労分析など        | 公式アプリでは得られないデータドリブンな改善示唆を提供 |

### 1.5 実装方針の注記

- **ディスカッション / 記事**: SNS（X 等）が既存のコミュニケーション手段として強いため、実装としての深掘りは行わない。ただし記事機能については、ダーツ関連の技術記事・攻略記事を自ら積極的に執筆・掲載する予定
- **分析機能の技術詳細**: 一部非公開 API を利用したデータ取得を行っているため、当該部分の技術詳細は設計書上では非公開とする
- **DARTSLIVE / PHOENIX 連携**: 両社は別会社であり、それぞれ独自のアプリ・データ形式を持つ。本サービスでは双方の連携を統合し、1 つのダッシュボードで横断的に閲覧できることを大きな価値としている
...
| 期間         | デイリー / 月間 / 年間                                       |
| 期限切れ処理 | 全期間で統一：未達成の期限切れ目標は自動削除（引き継ぎなし） |

#### F-024: 練習の意識ポイント

| 項目         | 内容                                                 |
| ------------ | ---------------------------------------------------- |
| 概要         | 練習時に意識するポイントを設定・表示                 |
| 上限         | 最大3つまで                                          |
| 表示場所     | トップ画面（ホーム）の目標セクション直下             |
| 操作         | テキスト入力で追加、「できた！」ボタンで確認後に削除 |
| データ保存先 | Firestore `users/{userId}/focusPoints/{docId}`       |
| 画面         | `/`                                                  |
...
- DARTSLIVEの100%スタッツ（PPD/MPR）からPHOENIXレーティング（Rt.1〜30）を換算表示
- 01/Cricket個別Rtと総合Rtを算出
- クラス（N〜GRAND MASTER）を併記

### 7.2 COUNT-UPセッション比較分析

- 複数セッションのラウンド別平均スコアを比較
- ラウンドパターン検知（コールドスタート型・失速型・安定型・中盤ピーク型・上昇型）
- ベスト/ワーストラウンドの特定

### 7.3 LINE COUNT-UP通知・練習メモ機能

- COUNT-UPスコア変動をLINE Flex Messageで自動通知
- 練習メモ・チャレンジ目標をスタッツと紐付けて記録

### 7.4 センサー分析（DL3ベクトル・速度）

- DARTSLIVEスマートセンサーからのグルーピング半径・投擲速度データを分析
- ミス方向の偏り検出（8方向ベクトル分析）
- アダプティブ刻み幅によるスピード分布分析

### 7.5 練習レコメンドエンジン

- ルールベースのAI練習レコメンデーション
- Bull率・アレンジ率・トリプル率・安定性・ミス方向などの指標から優先度付き推奨を生成
- カテゴリ別（bull/arrange/cricket/consistency/form/session/mental）に最大5件の推奨

---

## 8. v3.2 追加要件

### 8.1 ヘルスケア連携（HealthKit）

- Apple HealthKit から心拍・HRV・睡眠・歩数等10種のメトリクスを取得（Swift Capacitor プラグイン）
- ヘルスダッシュボードで一覧・推移グラフを表示
- ダーツ × ヘルス相関分析（ピアソン相関）

### 8.2 ヘルス相関分析のカウントアップ指標変更

- 01/Cricketは対戦相手やセパブルの影響を受け信頼性が低いため、相関分析の対象をPPD/MPRからカウントアップ平均スコアに変更
- カウントアップは自分一人の練習成果を反映し、ヘルスコンディションとの相関がより正確

### 8.3 練習の意識ポイント

- トップ画面に最大3つの意識ポイントを設定・表示
- 無意識にできるようになったら「できた！」で削除
- Firestore `users/{userId}/focusPoints` サブコレクションに保存

### 8.4 目標の期限切れ処理統一

- 月次目標の自動引き継ぎ（carryOver）を廃止
- daily / monthly / yearly 全ての期間で、未達成の期限切れ目標は自動削除に統一
